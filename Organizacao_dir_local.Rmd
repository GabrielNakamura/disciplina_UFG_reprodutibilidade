...
knitr::include_graphics("figs/directory_org.png")
```
...
A melhor referência para nomeação de arquivos é este [documento da Jenny Brian](https://www2.stat.duke.edu/~rcs46/lectures_2015/01-markdown-git/slides/naming-slides/naming-slides.pdf). Recomendo fortemente olhar este material para aprofundar as ideias que baseiam a nomeação de arquivos.
...
```{r echo=TRUE,eval=FALSE}
data_penguins <- read.csv("/Users/gabrielnakamura/OneDrive/Aulas/UFG_reprodutibilidade/disciplina_UFG_reprodutibilidade/data/data-penguins.csv")
```
...
```{r echo=TRUE,eval=FALSE}
library(here)
data_penguins <- read.csv(here("data", "data-penguins.csv"))
```
